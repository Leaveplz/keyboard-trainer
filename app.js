...
const letterRowSelect = document.querySelector("#letterRowSelect");
const letterRowLabel = document.querySelector("#letterRowLabel");
...
const letterCaseCheck = document.querySelector("#letterCaseCheck");
const letterCaseRow = document.querySelector("#letterCaseRow");
...
const sets = {
  ...
  capitalLetters: "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ".split(""),
  symbols: [".", ",", "!", "?", ":", ";", "-", "\"", "'", "(", ")", "+", "=", "*", "/", "@", "<", ">"],
...
const letterRows = {
  all: sets.letters,
  top: "йцукенгшщзхъ".split(""),
  middle: "фывапролджэ".split(""),
  bottom: "ячсмитьбю".split(""),
};

const phraseTopics = {
  hedgehog: [
    "Эх, - подумал ёжик, - как тихо и красиво сегодня в лесу!",
  ],
  mushroom: [
    "\"Смотри, это мухомор: яркий, красивый, но ядовитый!\" - сказала Маша.",
  ],
  stream: [
    "\"Пойдем к ручью: там, кажется, растёт земляника\", - предложил Петя.",
  ],
  forest: [
...
function symbolsMatch(typedSymbol, targetSymbol) {
  ...
  if (mode === "letters" && !letterCaseCheck.checked) {
    return typedSymbol.toLowerCase() === targetSymbol.toLowerCase();
  }
  return false;
}
...
function updateModeControls() {
  ...
  letterCaseRow.style.display = mode === "letters" ? "flex" : "none";
  letterRowLabel.style.display = mode === "letters" ? "grid" : "none";
...
function currentSymbolSet() {
  if (mode !== "letters") return sets[mode];
  const row = letterRows[letterRowSelect.value] || letterRows.all;
  if (!letterCaseCheck.checked) return row;
  return [...row, ...row.map((letter) => letter.toUpperCase())];
}
...
function makeBalloon(symbol) {
  ...
  if (mode === "letters" && symbol === symbol.toUpperCase()) {
    balloon.classList.add("capital");
  }
...
function markMiss(message) {
  ...
  document.querySelectorAll(".balloon").forEach((balloon) => {
    if (balloon.classList.contains("float")) return;
...
pauseButton.addEventListener("click", () => {
  if (!isPlaying) return;
  isPaused = !isPaused;
  screens.game.classList.toggle("paused", isPaused);
